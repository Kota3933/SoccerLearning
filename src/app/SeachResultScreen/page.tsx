...
export default function SeachResultScreen() {
		...
		const this_searchParams = useSearchParams();

		const preURL = 'http://localhost:3000/api/searchResult?';
		const word = this_searchParams.get('word');
		const positions = this_searchParams.getAll('position');
		const skills = this_searchParams.getAll('skill');

		const query = `word=${word}`
		const api_searchParams = new URLSearchParams(query);

		for(const position of positions) {
			api_searchParams.append('position', position);
		}
		for(const skill of skills) {
			api_searchParams.append('skill', skill);
		}

		const url = preURL + api_searchParams.toString();
		//SWRでurlにアクセス

		const fetcher = (url: string): Promise<JSON> => fetch(url).then(res => res.json());
